-- Aplica a correção final no backend (v4)
-- Remove a adição de Horas Extras no eventBreakdown para evitar duplicação com renderização explícita no frontend
-- Classifica DSR como 'INFO' para evitar que apareça no loop genérico de 'CREDIT', mas ainda apareça no loop específico de DSR
...
BEGIN
    ...
    LOOP
        ...
        -- IMPORTANTE: NÃO adicionar Horas Extras aqui, pois o Frontend já renderiza explicitamente via campos overtime_50/100
        -- Apenas DSR é adicionado pois o Frontend renderiza via filtro de nome 'DSR' no eventBreakdown
        -- E classificamos como 'INFO' para evitar duplicação no loop genérico de 'CREDIT'
