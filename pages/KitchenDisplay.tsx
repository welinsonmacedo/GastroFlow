...
export const KitchenDisplay: React.FC = () => {
  ...
  const handlePrintOrder = (order: any, tableNumber: number) => {
      ...
      const printContent = `
        ...
        <body>
            <div class="header">
                <div style="font-size: 1.5em; font-weight: bold;">MESA ${tableNumber}</div>
      ...
      return (
          <div className="h-full bg-slate-900 flex items-center justify-center p-4">
              <div className="text-center text-white space-y-6 max-w-md bg-slate-800 p-8 rounded-[2.5rem] shadow-2xl border border-white/5">
                  ...
                  <p className="text-slate-400 text-sm">O sistema manterá a tela ligada e emitirá alertas sonoros para novos pedidos.</p>
                  <button onClick={enableAudio} className="bg-emerald-600 hover:bg-emerald-500 text-white font-black py-5 px-8 rounded-2xl shadow-xl transition-all hover:scale-105 w-full text-xl flex items-center justify-center gap-3">
    ...
    <div className="h-full flex flex-col bg-slate-950 text-slate-100 p-4 overflow-hidden font-sans">
      <header className="mb-6 flex justify-between items-center shrink-0 bg-slate-900/50 p-4 rounded-3xl border border-white/5 backdrop-blur-md">
        ...
        <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 text-xs bg-emerald-500/10 px-4 py-2 rounded-2xl text-emerald-400 border border-emerald-500/20 font-black uppercase">
                <Zap size={14} className="animate-pulse" /> Online
            </div>
      ...
      <div className="flex-1 flex gap-5 overflow-x-auto pb-6 kds-scroll px-1">
        ...
        {activeOrders.map(order => {
          ...
          return (
            <div key={order.id} className={`min-w-[340px] max-w-[340px] bg-slate-900 rounded-[2rem] overflow-hidden border-2 flex flex-col shadow-2xl transition-all h-full ${isLate ? 'border-red-500 ring-4 ring-red-500/10' : 'border-white/5'}`}>
              <div className={`p-5 flex justify-between items-center shrink-0 ${isLate ? 'bg-red-600' : 'bg-slate-800'}`}>
                <div>
                ...
                <div className="flex items-center gap-2 text-xl font-black font-mono bg-black/30 px-3 py-1.5 rounded-2xl">
              ...
              <div className="bg-slate-850 p-3 border-b border-white/5 flex justify-between shrink-0 gap-2">
                  <button onClick={() => handlePrintOrder(order, table?.number || 0)} className="bg-slate-700 hover:bg-slate-600 p-3 rounded-2xl text-white transition-all"><Printer size={20} /></button>
