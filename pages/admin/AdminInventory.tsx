    ...
    <div className="space-y-6 animate-fade-in pb-20">
        ...
        {supplierModalOpen && (
            <Modal isOpen={supplierModalOpen} onClose={() => setSupplierModalOpen(false)} title="Gestão de Fornecedores" variant="page">
                <div className="space-y-8">
                    <form onSubmit={(e) => { e.preventDefault(); addSupplier(newSupplier as Supplier); setNewSupplier({name:'', phone:'', email:''}); showAlert({title:"Sucesso", message:"Fornecedor salvo!", type:'SUCCESS'}); }} className="bg-slate-50 p-6 rounded-2xl border-2 border-slate-200">
                        <h4 className="text-sm font-black text-slate-800 uppercase tracking-widest mb-4">Novo Fornecedor</h4>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="md:col-span-2"><label className="block text-[10px] font-bold text-slate-400 mb-1 uppercase">Nome Fantasia / Razão Social</label><input required className="w-full border-2 p-2.5 rounded-xl" value={newSupplier.name} onChange={e => setNewSupplier({...newSupplier, name: e.target.value})} placeholder="Ex: Distribuidora Silva LTDA" /></div>
                            <div><label className="block text-[10px] font-bold text-slate-400 mb-1 uppercase">WhatsApp</label><input className="w-full border-2 p-2.5 rounded-xl" value={newSupplier.phone} onChange={e => setNewSupplier({...newSupplier, phone: e.target.value})} placeholder="(00) 00000-0000" /></div>
                            <div className="md:col-span-2"><label className="block text-[10px] font-bold text-slate-400 mb-1 uppercase">E-mail de Contato</label><input className="w-full border-2 p-2.5 rounded-xl" value={newSupplier.email} onChange={e => setNewSupplier({...newSupplier, email: e.target.value})} placeholder="contato@fornecedor.com" /></div>
                            <div className="flex items-end"><Button type="submit" className="w-full py-2.5">Cadastrar</Button></div>
                        </div>
                    </form>
                        ...
                        {invState.suppliers.map(s => (
                            <div key={s.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col justify-between hover:border-blue-300 transition-colors group">
                                <div className="space-y-2">
                                    ...
                                    <div className="text-xs text-slate-400 flex items-center gap-2 truncate"><FileText size={12}/> {s.email}</div>
                                </div>
                            </div>
                        ))}
                    </div>
