...
import { Plus, Trash2, Edit, ArrowDown, Layers, ClipboardList, FileText, Star, CheckSquare, Square, Info } from 'lucide-react';
...
export const AdminInventory: React.FC = () => {
  ...
  const openEdit = (item: InventoryItem) => {
      ...
      if (item.type === 'COMPOSITE' && item.recipe) setInvRecipeStep(item.recipe.map(r => ({ ingredientId: r.ingredientId, qty: r.quantity })));
      else setInvRecipeStep([]);
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        <div className="flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border">
            <div>
                <h2 className="text-2xl font-bold text-gray-800">Estoque Geral</h2>
                <p className="text-sm text-gray-500">Insumos, Revendas e Adicionais.</p>
            </div>
                    ...
                    {invState.inventory.map(item => (
                        <tr key={item.id} className="hover:bg-gray-50">
                            ...
                            <td className="p-4 text-center">
                                <span className={`text-[10px] font-bold px-2 py-1 rounded ${item.type === 'INGREDIENT' ? 'bg-orange-100 text-orange-700' : item.type === 'RESALE' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'}`}>{item.type}</span>
                            </td>
            ...
            <Modal isOpen={!!editingInventory} onClose={() => setEditingInventory(null)} title="Cadastro de Estoque" variant="page">
                <form onSubmit={handleSaveInventoryItem} className="space-y-6">
                    <div className="bg-slate-50 p-4 rounded-xl border flex items-center justify-between">
                        <div>
                            <h4 className="font-bold text-sm">Disponível como Adicional?</h4>
                            <p className="text-xs text-gray-500">Marque para oferecer como opcional em outros pratos.</p>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="col-span-2">
                            ...
                            <input required className="w-full border p-3 rounded-lg" value={editingInventory.name} onChange={e => setEditingInventory({...editingInventory!, name: e.target.value})} />
                        </div>
                        <div>
                            <label className="block text-xs font-bold mb-1">Tipo</label>
                            <select className="w-full border p-3 rounded-lg bg-white" value={editingInventory.type} onChange={e => setEditingInventory({...editingInventory!, type: e.target.value as InventoryType})}>
                                <option value="INGREDIENT">Matéria Prima</option>
                                <option value="RESALE">Revenda</option>
                                <option value="COMPOSITE">Composto (Prato)</option>
                            </select>
                        </div>
                        <div>
                            <label className="block text-xs font-bold mb-1">Custo Médio (R$)</label>
                            <input type="number" step="0.01" className="w-full border p-3 rounded-lg" value={editingInventory.costPrice} onChange={e => setEditingInventory({...editingInventory!, costPrice: parseFloat(e.target.value)})} />
                        </div>
                    </div>
