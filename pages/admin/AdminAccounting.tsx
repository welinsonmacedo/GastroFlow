...
import { Loader2, RefreshCcw, Printer, PieChart, TrendingUp, CheckCircle2, ArrowUpRight, AlertCircle } from 'lucide-react';
...
export const AdminAccounting: React.FC = () => {
  ...
  const fetchDRE = useCallback(async () => {
      ...
      try {
          ...
          const [transRes, expsRes] = await Promise.all([
            supabase.from('transactions').select(`*, orders ( items:order_items ( quantity, product_price, product_cost_price ) )`).eq('tenant_id', state.tenantId).gte('created_at', start).lte('created_at', end),
            supabase.from('expenses').select('*').eq('tenant_id', state.tenantId).gte('due_date', dateStart).lte('due_date', dateEnd)
          ]);

          let grossRev = 0, cmvTotal = 0, saloonSales = 0, posSales = 0;
          ...
          const taxes = grossRev * 0.06; // Simulação fixa 6% Simples
          ...
          expsRes.data?.forEach((e: any) => {
              if (['Impostos', 'Taxas Bancárias'].includes(e.category)) finExpenses += e.amount;
              else {
                  opExpenses += e.amount;
    ...
    <div className="space-y-6 animate-fade-in pb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className={`p-6 rounded-2xl border-2 bg-white shadow-sm flex flex-col ${cmvPerc > 35 ? 'border-red-100' : 'border-emerald-100'}`}>
                <div className="flex justify-between items-start mb-2"><span className="text-gray-400 text-[10px] font-black uppercase tracking-widest">CMV Real</span>{cmvPerc > 35 ? <ArrowUpRight className="text-red-500" /> : <CheckCircle2 className="text-emerald-500" />}</div>
            ...
            <div className={`p-6 rounded-2xl border-2 bg-white shadow-sm flex flex-col border-blue-100`}>
                <div className="flex justify-between items-start mb-2"><span className="text-gray-400 text-[10px] font-black uppercase tracking-widest">Margem Bruta</span><TrendingUp className="text-blue-500" /></div>
            ...
            <div className={`p-6 rounded-2xl border-2 bg-white shadow-sm flex flex-col ${profitPerc < 10 ? 'border-orange-100' : 'border-emerald-100'}`}>
                <div className="flex justify-between items-start mb-2"><span className="text-gray-400 text-[10px] font-black uppercase tracking-widest">Lucratividade</span>{profitPerc < 10 ? <AlertCircle className="text-orange-500" /> : <TrendingUp className="text-emerald-500" />}</div>
        ...
        <div className="bg-white rounded-2xl shadow-xl border overflow-hidden print:border-none print:shadow-none">
            <div className="bg-slate-900 p-8 text-white flex justify-between items-end">
                <div><h1 className="text-3xl font-black tracking-tighter uppercase mb-1">DRE Gerencial</h1><p className="text-slate-400 text-xs font-bold uppercase tracking-widest">{state.theme.restaurantName}</p></div>
                <div className="text-right"><p className="text-slate-500 text-[10px] font-black uppercase mb-1">Período</p><p className="text-sm font-mono">{new Date(dateStart).toLocaleDateString()} — {new Date(dateEnd).toLocaleDateString()}</p></div>
            </div>
            ...
            <div className="p-8 max-w-4xl mx-auto">
                <div className="space-y-1">
                    ...
                    <Row label="(+) Receita Bruta (Vendas)" value={data.grossRevenue} />
                    <Row label="Vendas no Salão / Mesas" value={data.saloonSales} indent />
                    <Row label="Vendas Balcão / PDV" value={data.posSales} indent />
                    <Row label="(-) Impostos s/ Faturamento (Simples/ISS)" value={data.taxes} isNegative />
                    ...
                    <Row label="(-) Custo de Mercadoria Vendida (CMV)" value={data.cmv} isNegative />
                ...
                <div className={`mt-12 p-6 rounded-2xl flex items-center justify-between border-4 ${data.netIncome >= 0 ? 'bg-emerald-50 border-emerald-200' : 'bg-red-50 border-red-200'}`}>
                    <div><h4 className={`text-xl font-black uppercase ${data.netIncome >= 0 ? 'text-emerald-800' : 'text-red-800'}`}>{data.netIncome >= 0 ? 'Lucro Líquido no Período' : 'Prejuízo no Período'}</h4><p className="text-slate-500 text-sm font-medium">Resultado final após todas as deduções.</p></div>
                    <div className={`text-4xl font-black ${data.netIncome >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>R$ {data.netIncome.toFixed(2)}</div>
                </div>
            </div>
        </div>
    </div>
