...
export const RestaurantProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const dispatch = async (action: any) => {
    ...
    switch (action.type) {
        case 'UPDATE_THEME': await supabase.from('tenants').update({ theme_config: action.theme }).eq('id', tenantId); localDispatch(action); break;
        case 'UPDATE_BUSINESS_INFO': await supabase.from('tenants').update({ business_info: action.info }).eq('id', tenantId); localDispatch(action); break;
        default: break; 
    }
  };
