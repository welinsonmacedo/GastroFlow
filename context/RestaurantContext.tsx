...
import { 
  ...
  RestaurantBusinessInfo, PlanLimits, Role, User, ProductType, OrderStatus, POSSaleData 
} from '../types';
...
const restaurantReducer = (state: RestaurantState, action: any): RestaurantState => {
  switch (action.type) {
    ...
    case 'SET_INITIAL_DATA': return { ...state, ...action.payload, isLoading: false };
    case 'SET_TABLES': return { ...state, tables: action.payload };
    case 'SET_PRODUCTS': return { ...state, products: action.payload };
    case 'SET_ORDERS': return { ...state, orders: action.payload };
    case 'SET_SERVICE_CALLS': return { ...state, serviceCalls: action.payload };
  ...
  const dispatch = async (action: RestaurantAction) => {
    ...
    switch (action.type) {
      ...
      case 'PROCESS_POS_SALE':
        const { data: posOrder } = await supabase.from('orders').insert({ tenant_id: tenantId, status: 'DELIVERED', is_paid: true, customer_name: action.sale.customerName }).select().single();
        if (posOrder) {
          const posItems = action.sale.items.map(i => {
            const product = state.products.find(p => p.id === i.productId);
            return {
              tenant_id: tenantId, order_id: posOrder.id, product_id: i.productId, quantity: i.quantity, notes: i.notes || '', status: 'DELIVERED',
              product_name: product?.name || 'Item', product_type: product?.type || 'BAR',
              product_price: product?.price || 0, product_cost_price: product?.costPrice || 0
            };
          });
          await supabase.from('order_items').insert(posItems);
          await supabase.from('transactions').insert({
            tenant_id: tenantId, order_id: posOrder.id, amount: action.sale.totalAmount, method: action.sale.method, items_summary: `Venda PDV: ${action.sale.customerName}`
          });
        }
        break;
      ...
      case 'PROCESS_PAYMENT':
        ...
        await supabase.from('transactions').insert({
          tenant_id: tenantId, table_id: action.tableId, table_number: table?.number || 0, amount: action.amount, method: action.method, items_summary: `Mesa ${table?.number}`
        });
      ...
      case 'ADD_TABLE': 
        const n = state.tables.length > 0 ? Math.max(...state.tables.map(t => t.number)) + 1 : 1;
        await supabase.from('restaurant_tables').insert({ tenant_id: tenantId, number: n, status: 'AVAILABLE' }); 
        break;
      ...
      case 'ADD_PRODUCT_TO_MENU': 
        await supabase.from('products').insert({
            ...
            is_extra: action.product.isExtra, linked_extra_ids: action.product.linkedExtraIds
        }); 
        break;
      case 'UPDATE_PRODUCT':
        await supabase.from('products').update({
            name: action.product.name, price: action.product.price, category: action.product.category, 
      ...
      case 'UPDATE_THEME': await supabase.from('tenants').update({ theme_config: action.theme }).eq('id', tenantId); baseDispatch(action); break;
      case 'UPDATE_BUSINESS_INFO': await supabase.from('tenants').update({ business_info: action.info }).eq('id', tenantId); baseDispatch(action); break;
      case 'UNLOCK_AUDIO': baseDispatch(action); break;
    }
  };
