...
const initialState: RestaurantState = {
  ...
  theme: { primaryColor: '#2563eb', backgroundColor: '#fff', fontColor: '#1f2937', logoUrl: '', restaurantName: 'Carregando...' },
  businessInfo: {}, 
  serviceCalls: [], users: [],
  audioUnlocked: false
};
  ...
  const dispatch = async (action: any) => {
    ...
    switch (action.type) {
        ...
        case 'PROCESS_POS_SALE':
            await supabase.rpc('process_pos_sale', { p_tenant_id: tenantId, p_customer_name: action.sale.customerName, p_total_amount: action.sale.totalAmount, p_method: action.sale.method, p_items: action.sale.items });
            break;
        case 'PROCESS_PAYMENT':
            ...
            await supabase.from('transactions').insert({ tenant_id: tenantId, table_id: action.tableId, amount: action.amount, method: action.method, items_summary: `Mesa ${table?.number}`, cashier_name: 'Caixa' });
            break;
