...
const TenantNavigation = ({ isOpen, setIsOpen }: { isOpen: boolean, setIsOpen: (v: boolean) => void }) => {
    ...
    // AJUSTE: Oculta Sidebar em rotas operacionais puras (sem /admin) e rotas públicas
    if (['/client', '/login', '/manual', '/waiter', '/kitchen', '/cashier'].some(path => location.pathname.startsWith(path))) return null;
    
    if (!authState.currentUser) return null;

    const role = authState.currentUser.role;

    // 1. Módulos Operacionais (Apps) - Links apontam para /admin/... para manter a sidebar para o Admin
    const appLinks = [
    ...
    return (
        <div className="h-full flex flex-row bg-gray-50 overflow-hidden relative">
            ...
            <div className="flex-1 overflow-hidden relative flex flex-col w-full">
                {/* Mobile Header Toggle - Oculta nas rotas operacionais puras */}
                {!['/waiter', '/kitchen', '/cashier'].some(path => location.pathname.startsWith(path)) && (
                ...
                <div className="flex-1 overflow-y-auto">
                    <Routes>
                        ...
                        {/* Rotas Operacionais Raiz (Sem Sidebar) */}
