...
const TenantNavigation = ({ isOpen, setIsOpen }: { isOpen: boolean, setIsOpen: (v: boolean) => void }) => {
    ...
    // Dupla verificação de segurança, embora o pai agora controle a renderização
...
const TenantApp = () => {
    ...
    // Verificação precisa do início da rota
    ...
    return (
        <div className="h-full flex flex-row bg-gray-50 overflow-hidden relative">
            {/* CONDICIONAL: A Sidebar só é renderizada se shouldHideSidebar for FALSE */}
            {!shouldHideSidebar && (
                <TenantNavigation isOpen={isSidebarOpen} setIsOpen={setIsSidebarOpen} />
            )}
            
            <div className={`flex-1 overflow-hidden relative flex flex-col w-full transition-all`}>
