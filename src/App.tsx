...
const ProtectedRestaurantRoute = ({ children, allowedRoles, requiredRoute, requiredFeature }: PropsWithChildren<ProtectedRouteProps>) => {
    ...
    if (requiredFeature && restState.planLimits) {
        // Se a feature for "allowCashier", verificamos se QUALQUER UM dos sub-módulos está ativo
        if (requiredFeature === 'allowCashier') {
             if (!restState.planLimits.allowPos && !restState.planLimits.allowDelivery && !restState.planLimits.allowCashControl) {
...
const TenantNavigation = ({ isOpen, setIsOpen }: { isOpen: boolean, setIsOpen: (v: boolean) => void }) => {
    ...
    const appLinks = [
        ...
        // ANY_CASHIER significa que se tiver PDV, Delivery OU Caixa, aparece o link
        { to: "/cashier", icon: <DollarSign size={20}/>, label: "Caixa", requires: 'ANY_CASHIER', roles: [Role.CASHIER, Role.ADMIN] },
    ];
    ...
    const filterLinks = (links: any[]) => links.filter(link => {
        ...
        // Verifica se QUALQUER modulo de caixa está ativo para exibir o link
        if (link.requires === 'ANY_CASHIER') {
            if (!planLimits.allowPos && !planLimits.allowDelivery && !planLimits.allowCashControl) return false;
        }
            ...
            <div className="flex-1 overflow-hidden relative flex flex-col w-full">
                ...
                <div className="flex-1 overflow-y-auto">
                    <Routes>
                        ...
                        {/* Caixa agora usa a flag "allowCashier" que foi adaptada para "ANY_CASHIER" logicamente */}
