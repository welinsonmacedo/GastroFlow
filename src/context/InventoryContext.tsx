...
export const InventoryProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchData = useCallback(async () => {
        ...
        if (invRes.data) {
            ...
            const mappedSuppliers = (suppRes.data || []).map((s: any) => ({
                ...
                email: s.email, cnpj: s.cnpj, ie: s.ie, cep: s.cep, address: s.address,
                number: s.number, complement: s.complement, city: s.city, state: s.state
            }));
  ...
  const addInventoryItem = async (item: InventoryItem) => {
      ...
      const payload = {
          tenant_id: tenantId, name: item.name, unit: item.unit, 
          quantity: item.quantity || 0, min_quantity: item.minQuantity || 0, 
          cost_price: item.costPrice || 0, sale_price: item.salePrice || 0, 
          type: item.type, category: item.category, image: item.image,
          is_extra: item.isExtra, target_categories: item.targetCategories
      };
  ...
  const updateInventoryItem = async (item: InventoryItem) => {
      ...
      const payload = {
          name: item.name, unit: item.unit, min_quantity: item.minQuantity || 0,
          cost_price: item.costPrice || 0, sale_price: item.salePrice || 0,
          image: item.image, type: item.type, category: item.category,
          is_extra: item.isExtra, target_categories: item.targetCategories
      };
  ...
  const deleteInventoryItem = async (itemId: string) => {
      ...
      if (error) {
          if (error.code === '23503') throw new Error("Não é possível excluir este item pois ele está vinculado a produtos, receitas ou histórico de vendas.");
          throw error;
      }
  ...
  const addSupplier = async (supplier: Supplier) => {
      if(!tenantId) return;
      const payload = {
          tenant_id: tenantId, name: supplier.name, contact_name: supplier.contactName,
          phone: supplier.phone, email: supplier.email, cnpj: supplier.cnpj, ie: supplier.ie,
          cep: supplier.cep, address: supplier.address, number: supplier.number,
          complement: supplier.complement, city: supplier.city, state: supplier.state
      };
  ...
  const processPurchase = async (purchase: PurchaseEntry) => {
      ...
      try {
          ...
          for (const item of purchase.items) {
              ...
              await supabase.from('inventory_items').update({ quantity: newQty, cost_price: effectiveUnitCost }).eq('id', item.inventoryItemId);
              await supabase.from('inventory_logs').insert({ tenant_id: tenantId, item_id: item.inventoryItemId, type: 'IN', quantity: item.quantity, reason: `Compra Nota ${purchase.invoiceNumber}`, user_name: 'Admin' });
          }

          const supplierName = state.suppliers.find(s => s.id === purchase.supplierId)?.name || 'Fornecedor';
          await supabase.from('expenses').insert({
              tenant_id: tenantId, description: `Compra Nota ${purchase.invoiceNumber} (${supplierName})`,
              amount: purchase.totalAmount, category: 'Fornecedor', due_date: purchase.date,
              is_paid: false, payment_method: 'BANK', supplier_id: purchase.supplierId
          });
