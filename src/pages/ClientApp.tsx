...
export const ClientApp: React.FC = () => {
    ...
    const getGroupedExtras = (product: Product) => {
        ...
        // Agrupar por categoria do produto extra
            ...
            <header className="bg-white/70 backdrop-blur-xl shadow-sm sticky top-0 z-40 border-b border-white/20">
                <div className="flex justify-between items-center p-4 max-w-2xl mx-auto">
                    ...
                    <div className="flex gap-2">
                        {/* Botão Meus Pedidos (Status) */}
                        <button 
                            onClick={() => setView('STATUS')} 
                            className={`p-3 rounded-2xl transition-all ${view === 'STATUS' ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-100 text-slate-600 hover:bg-gray-200'}`}
                            title="Meus Pedidos"
                        >
                            <Clock size={20} />
                        </button>
                                        ...
                                        {items.map(product => (
                                            ...
                                            >
                                                <div className={`relative shrink-0 overflow-hidden bg-gray-50 ${isGrid ? 'w-full h-48' : 'w-24 h-24 sm:w-32 sm:h-32 rounded-3xl'}`}>
                                                    <img src={product.image || 'https://via.placeholder.com/300x300?text=Foto'} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt={product.name} />
                            ...
                            <div className="relative">
                                ...
                                <div className="absolute bottom-0 left-0 w-full p-4 text-white">
                                    <div className="flex justify-between items-end">
                                        <h3 className="font-black text-2xl truncate pr-4 text-shadow leading-none">{selectedProduct.name}</h3>
                                    ...
                                    {selectedProduct.description && (
                                        <p className="text-xs text-gray-200 mt-2 font-medium line-clamp-2 leading-relaxed">{selectedProduct.description}</p>
                                    )}
                                </div>
                            </div>
                                    ...
                                    <div className="border-t border-gray-100 py-6 space-y-6">
                                        {Object.entries(getGroupedExtras(selectedProduct)).map(([category, extras]) => (
                                            <div key={category} className="space-y-3">
                                                <label className="block text-xs font-black text-slate-800 uppercase tracking-wider flex items-center gap-1 border-b border-gray-100 pb-1">
                                                    {category}
                                                </label>
                                                <div className="space-y-2">
                                                    {extras.map(extra => {
