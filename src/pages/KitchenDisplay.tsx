...
export const KitchenDisplay: React.FC = () => {
      ...
      return (
          <div className="h-full bg-slate-900 flex items-center justify-center p-4">
              <div className="text-center text-white space-y-6 max-w-md bg-slate-800 p-8 rounded-[2.5rem] shadow-2xl border border-white/5">
                  <div className="bg-slate-900 p-6 rounded-full inline-block mb-4 border-2 border-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.3)]"><ChefHat size={64} className="text-emerald-500" /></div>
                  ...
                  <button onClick={enableAudio} className="bg-emerald-600 hover:bg-emerald-500 text-white font-black py-5 px-8 rounded-2xl shadow-xl w-full text-xl flex items-center justify-center gap-3"><Volume2 size={28} /> ATIVAR COZINHA</button>
              </div>
          </div>
      );
  }
  ...
  return (
    <div className="h-full flex flex-col bg-slate-950 text-slate-100 p-4 overflow-hidden font-sans">
      ...
      <div className="flex-1 flex gap-4 overflow-x-auto pb-6 kds-scroll px-1">
        {activeOrders.length === 0 && <div className="w-full h-full flex flex-col items-center justify-center text-slate-600 gap-4"><div className="bg-slate-900 p-10 rounded-full border border-white/5 opacity-50"><ChefHat size={80} className="text-slate-700"/></div><div className="text-2xl font-black uppercase tracking-widest opacity-30">Tudo limpo, Chef!</div></div>}
            ...
            <div key={order.id} className={`min-w-[320px] max-w-[320px] bg-slate-900 rounded-[2rem] overflow-hidden border-2 flex flex-col shadow-2xl transition-all h-full ${isLate ? 'border-red-500 ring-4 ring-red-500/10' : (isDelivery ? 'border-orange-500' : (isPDV ? 'border-purple-500' : 'border-white/10'))}`}>
              ...
              <div className={`p-4 flex justify-between items-center shrink-0 ${isLate ? 'bg-red-600' : (isDelivery ? 'bg-orange-600' : (isPDV ? 'bg-purple-600' : 'bg-slate-800'))}`}>
                <div className="overflow-hidden">
                    {isDelivery ? (
                        <>
                            <div className="flex items-center gap-2 mb-1"><Bike size={20} className="text-white"/><span className="font-black text-xl tracking-tighter uppercase truncate">{order.deliveryInfo?.platform || 'DELIVERY'}</span></div>
                        ...
                        <>
                            <div className="flex items-center gap-2 mb-1"><span className="font-black text-xl tracking-tighter uppercase truncate">BALCÃO</span></div>
